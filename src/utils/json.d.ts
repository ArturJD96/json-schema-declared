 ...
 * * `array` as Json[] type.
 */
export type Json = Leaf | Leaf[] | JsonObject // TO DO: rename to JsonNode
export type JsonObject = { [k: string]: Json | Json[] } // to do: [k: string]
export type JsonArray = { [k: number]: Leaf | JsonObject } // To DO: implement it fully
export type Leaf = boolean | null | number | string
export type JsonKey = string | number

/**
 * Utility type for distinguishing schemas vs. ordinary JSON objects.
 */
export type MinimalJsonSchema = JsonObject & { "$schema": string }
